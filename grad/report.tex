...
Ses deux sous-équipes plus d'autres d'\gls{ocd} travaillent ensemble pour
...
\begin{itemize}
    ...
    \item Une fois la spécification rédigée, ajustée et approuvée par le client,
        ...
        aussi, lorsque cela peut s'appliquer, de vérifier de bon fonctionnement
    ...
    \item Quand l'implémentation arrive vers sa fin, le \gls{ocd-build} commence
        a rédiger en parallèle un \gls{dtv} par site déployé. Il regroupe un
        ...
        fait partie de l'architecture globale.  Les différents \gls{dtv} sont
    ...
    \item Après que le \gls{dtv} d'un est bien validé et que son implémentation
        est terminée par le \gls{ocd-build}, une \gls{vabf} est plannifiée et
        effectuée avec le client. Il s'agit d'une session lors de laquelle tous
        les tests d'acceptation du \gls{dtv} associé sont exécutés et leur
        résultats respectifs rapportés dans le document. Si un nombre
        ...
        estimées non courantes les systèmes ne fonctionnent pas comme attendus,
        il est alors possible de considérer la \gls{vabf} comme un succès avec
        quelques retenues tout de même. Les problèmes dans les jours qui suivent
        reglés par le \gls{ocd-build} et vérifiés en dehors d'une seconde
        \gls{vabf}.

    \item Si les résultats de tous les tests sont des succès, la \gls{vabf} est
        bien complétée, les ingénrieurs \gls{ocd-build} sont libérés du projet
        et le site en question entre dans une période dite de \gls{vsr}. Pendant
        ...
        la journée par exemple, ou lorsque les systèmes sont soumis à des fortes
    ...
    \item Quand toutes ces étapes-là sont bien complétée, le projet est en
